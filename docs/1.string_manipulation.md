...
- `(log|chk)` means to match either `log` or `chk`. The `|` functions as an OR operator and the round brackets `()` declare a subexpression or group.
...
We can make use of `str_extract` on the `Taxon` column with a regex pattern to extract out the text that is related to genus. We know that ranks are prefixed by a single-letter followed by two underscores - "g__" in the case of species, and ranks are separated by a semicolon, so we can use this information in constructing out pattern: `(g__[\\w]+;)`

!!! r-project "code"

    ```r linenums="1"
    tax %>% mutate(genus = str_extract(Taxon, "g__[\\w]+;"))
    ```
--------------------------------------------
