...
In this lesson, we will learn how to use functions from the `stringr` package to manipulate and extract strings from taxonomic lineages in the `tax` data frame. The `stringr` package makes use of regular expressions (patterns) to do matching on string/character data. Key functions in this package that we'll make use of are:
- '`str_detect()` which allows us to do filtering based on the presence of a string that matches our pattern,
- `str_extract()` which allows us to create patterns to extract specific strings that match, and 
- `str_replace()` which allows us to replace strings with other strings based on a matching pattern.
...
- `.*` means to match anything `.` $\ge$ 0 times `*`. The `.` means anything, the `*` quantifies this to be 0 or more times
- `_` is just an underscore
- `(log|chk)` means to match either `log` or `chk`. The `|` functions as an OR operator and the round brackets `()` declare a subexpression or group. Groups are numbered and can be referred to.
- `\\.` matches a literal dot `.` - characters that have special meanings in regex need to be escaped with a backslash, in R, this requires use of a double backslash (`\\`).

Additional useful defined patterns:

- You can use `^` within the square brackets to "negate/invert the match. e.g. `[^0-2]` would match anything that wasn't the digits 0, 1 or 2.
- `\\w` "word" characters (`0-9`, `a-z`, `A-Z`, and underscore `_`) e.g not whitespace or punctuation
- `\\d` digit characters (shorthand for `[0-9]`)
- Anchoring:
  - `^` anchors the pattern to the start of the string e.g. "^[0-9]" would match anything starting with a digit (note the `^` is outisde the `[]`'s)
  - `$` anchors the pattern to the end of the string e.g `txt$` would match anything ending in "txt"
...
We can see from this that this data isn't in a _tidy_ format - the `Taxon` column has multiple pieces of information and this will make it challenging to do further analysis or visualisation on it.
...
We can make use of `str_extract` on the `Taxon` column with a regex pattern to extract out the text that is related to genus into a new column. We know that ranks are prefixed by a single-letter followed by two underscores - "g__" in the case of genus, and ranks are separated by a semicolon, so we can use this information in constructing our pattern: `(g__[\\w]+;)`

!!! r-project "code"

    ```r linenums="1"
    tax <- tax %>% mutate(genus = str_extract(Taxon, "g__[\\w]+;"))
    ```

And this could be further cleaned up by making use of `str_remove_all()` to get rid of the "g__" prefix and the trailing semicolon


!!! r-project "code"

    ```r linenums="1"
    tax <- tax %>% mutate(genus = str_remove_all(genus, "^g__|;$"))
    ```

   
**UP TO HERE UP TO HERE UP TO HERE**
